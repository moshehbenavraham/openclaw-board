...
export function GlobalBugsOverlay() {
	...
	useEffect(() => {
		...
		const system = new BugSystem(
			window.innerWidth / BUGS_ZOOM,
			window.innerHeight / BUGS_ZOOM,
			initial.count,
		);
		system.setEnabled(initial.enabled);
		system.setTargetCount(
			initial.count,
			window.innerWidth / BUGS_ZOOM,
			window.innerHeight / BUGS_ZOOM,
		);
		...
		const applyConfig = () => {
			...
			systemRef.current.setTargetCount(
				cfg.count,
				window.innerWidth / BUGS_ZOOM,
				window.innerHeight / BUGS_ZOOM,
			);
		...
		const onLogoDragStop = () => {
			...
			window.dispatchEvent(
				new CustomEvent("openclaw-logo-carry-progress", {
		...
		window.addEventListener(
			...
			onConfigChanged as EventListener,
		);
		window.addEventListener(
			"openclaw-logo-drag-start",
			onLogoDragStart as EventListener,
		);
		window.addEventListener(
			"openclaw-logo-drag-stop",
			onLogoDragStop as EventListener,
		);
		...
		const tick = (ts: number) => {
			...
			if (ctx && systemRef.current) {
				systemRef.current.setCursor(
					mouseRef.current.x / BUGS_ZOOM,
					mouseRef.current.y / BUGS_ZOOM,
				...
				window.dispatchEvent(
					new CustomEvent("openclaw-logo-carry-progress", {
						detail: {
							dx: carry.dx * BUGS_ZOOM,
							dy: carry.dy * BUGS_ZOOM,
							angle: carry.angle,
							hidden: carry.hidden,
							active: carry.active,
						},
