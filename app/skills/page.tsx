...
export default function SkillsPage() {
	...
	useEffect(() => {
		...
		fetch(
			`/api/skills/content?source=${encodeURIComponent(selectedSkill.source)}&id=${encodeURIComponent(selectedSkill.id)}`,
			{
	...
	const filtered = skills.filter((skill) => {
		...
		if (filter === "extension" && !skill.source.startsWith("extension:"))
			return false;
	...
	const sourceLabel = (source: string) => {
		...
		if (source.startsWith("extension:"))
			return source.replace("extension:", `${t("skills.extension")}:`);
	...
	const selectedSkillCacheKey = selectedSkill
		? `${selectedSkill.source}:${selectedSkill.id}`
		: "";
	const selectedSkillContent = selectedSkill
		? skillContent[selectedSkillCacheKey]
		: "";
	...
	const extensionCount = skills.filter((skill) =>
		skill.source.startsWith("extension:"),
	).length;
		...
		<main className="min-h-screen p-4 md:p-8 max-w-6xl mx-auto">
			<div className="flex flex-col gap-3 mb-6 md:flex-row md:items-center md:justify-between">
				<div>
					...
					<p className="text-[var(--text-muted)] text-sm mt-1">
						{skills.length} {t("skills.count")} ({t("skills.builtin")}{" "}
						{builtinCount} / {t("skills.extension")} {extensionCount} /{" "}
						{t("skills.custom")} {customCount})
					</p>
				</div>
			...
			<div className="flex flex-col gap-3 mb-6 md:flex-row md:items-center">
				<div className="flex flex-wrap rounded-lg border border-[var(--border)] overflow-hidden">
					{(["all", "builtin", "extension", "custom"] as const).map(
						(nextFilter) => (
							<button
								key={nextFilter}
					...
					filtered.map((skill) => (
						...
						>
							<div className="flex items-start justify-between mb-2 gap-2">
								...
								<span
									className={`px-2 py-0.5 rounded-full text-[10px] font-medium shrink-0 ${sourceBadgeClass(skill.source)}`}
								>
