...
function BarChart({
	...
	const width = Math.max(
		600,
		data.length * (bars.length * 24 + 16) + padding.left + padding.right,
	);
					...
					return (
						<g key={d.date}>
							{bars.map((b, bi) => {
								...
								const x =
									groupX +
									(groupWidth - bars.length * barWidth) / 2 +
									bi * barWidth;
...
function ResponseTimeChart({
	...
	const width = Math.max(
		600,
		filtered.length * 40 + padding.left + padding.right,
	);
	const chartW = width - padding.left - padding.right;
	const chartH = height - padding.top - padding.bottom;
		...
		<main className="min-h-screen p-4 md:p-8 max-w-6xl mx-auto">
			...
			{range === "daily" && (
				<div className="p-5 rounded-xl border border-[var(--border)] bg-[var(--card)]">
					...
					<ResponseTimeChart
						data={currentData}
						noDataText={t("stats.noResponseData")}
					/>
				</div>
			)}
		</main>
