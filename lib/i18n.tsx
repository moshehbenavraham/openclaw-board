...
const englishTranslations: Record<string, string> = {
	...
	"agent.modelApplyHint":
		"Saving will restart the Gateway automatically to apply the new model.",
	...
	"models.colTest": "Test",
	"models.noExplicitModels":
		"No explicit model definitions (inferred from provider name)",
	...
	"sessions.testHint":
		"Tests verify agent responsiveness only. Messages will not appear in Feishu or Discord chats.",
...
export function I18nProvider({ children }: { children: ReactNode }) {
	...
	const t = useCallback(
		(key: string) =>
			translations[locale]?.[key] ?? englishTranslations[key] ?? key,
		[locale],
	);
