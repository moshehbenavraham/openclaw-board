...
export class BugSystem {
	...
	constructor(
		worldWidth: number,
		worldHeight: number,
		initialCount = BUG_DEFAULT_COUNT,
	) {
		this.targetCount = clamp(initialCount, 0, BUG_MAX_COUNT);
		this.grid = new BugSpatialGrid(BUG_GRID_CELL_SIZE);
		this.pheromones = new BugPheromoneField(
			worldWidth,
			worldHeight,
			BUG_PHEROMONE_CELL_SIZE,
		);
		for (let i = 0; i < this.targetCount; i++)
			this.bugs.push(this.createBug(worldWidth, worldHeight));
	}
	...
	startLogoCarry(
		...
		while (this.bugs.length < need) {
			this.bugs.push(
				this.createBug(
					Math.max(targetX, startX) + 120,
					Math.max(targetY, startY) + 120,
				),
			);
		}
	...
	update(dt: number, worldWidth: number, worldHeight: number): void {
		...
		for (const bug of this.bugs) {
			...
			if (bug.wanderTimer <= 0) {
				...
				bug.wanderTargetHeading =
					bug.heading +
					(randomRange(-wanderRange, wanderRange) * Math.PI) / 180;
			...
			const carryHeading =
				bug.isCarrier && this.logoCarry.active
					? this.computeCarryHeading(bug)
					: null;
			const boidsHeading =
				carryHeading ??
				this.computeBoidsHeading(bug, neighbors, worldWidth, worldHeight, dt);
			const blendedTargetHeading = boidsHeading ?? bug.wanderTargetHeading;
			const turnRate = bug.isCarrier
				? 7.5
				: bug.behaviorType === "loner"
					? 5.0
					: 3.5;
			...
			if (bug.behaviorType === "edgeDweller") {
				...
				const edgeDir =
					Math.abs(bug.x - centerX) > Math.abs(bug.y - centerY)
						? Math.atan2(0, nearestEdgeX - bug.x)
						: Math.atan2(nearestEdgeY - bug.y, 0);
				bug.heading +=
					normalizeRadians(edgeDir - bug.heading) * Math.min(1, dt * 1.4);
			}

			const speed = bug.isCarrier
				? Math.max(bug.speed, BUG_MAX_SPEED * 0.95) *
					this.getCarrySpeedMultiplier(bug)
				: bug.speed;
			...
			this.pheromones.deposit(
				...
				BUG_PHEROMONE_DEPOSIT_PER_SEC *
					dt *
					this.getDepositScale(bug.behaviorType),
				BUG_PHEROMONE_MAX_STRENGTH,
			);
			...
			if (bug.speed > 0.1) {
				bug.gaitTimer +=
					BUG_LEG_UPDATE_INTERVAL_SEC * (bug.speed / BUG_MAX_SPEED) * 8.0;
				if (bug.speed < BUG_MAX_SPEED * 0.5)
					bug.gaitTimer += BUG_LEG_UPDATE_INTERVAL_SEC * 0.8;
				if (bug.speed > BUG_MAX_SPEED * 1.2)
					bug.gaitTimer += BUG_LEG_UPDATE_INTERVAL_SEC * 2.0;
	...
	private reconcileCount(worldWidth: number, worldHeight: number): void {
		while (this.bugs.length < this.targetCount)
			this.bugs.push(this.createBug(worldWidth, worldHeight));
	...
	private computeBoidsHeading(
		...
		const sample = this.pheromones.sampleDirection(
			bug.x,
			bug.y,
			BUG_TRAIL_FOLLOW_RADIUS,
		);
		...
		if (!canFollowTrail) {
			...
			!bug.followingTrail &&
			sample.strength > BUG_PHEROMONE_MIN_DETECT &&
			Math.random() < enterFollowP
		) {
		...
		if (canFollowTrail && sample.strength > BUG_PHEROMONE_MIN_DETECT) {
			...
			if (plen > 0.0001) {
				const followW = bug.followingTrail
					? BUG_TRAIL_STEERING_WEIGHT
					: BUG_PHEROMONE_FOLLOW_WEIGHT;
		...
		if (this.cursorActive && !bug.isCarrier) {
			...
			if (distSq > 0.0001 && distSq < radSq) {
				...
				const repulse =
					BUG_CURSOR_REPULSION_STRENGTH *
					(1 - dist / BUG_CURSOR_REPULSION_RADIUS);
				fx += nx * repulse * 10.0;
				fy += ny * repulse * 10.0;
			}
		}
	...
	private updateLogoCarryVisual(
		...
		const targetHeading =
			Math.atan2(toTargetY, toTargetX) + this.logoCarry.detourAngle;
		...
		for (const b of this.bugs) {
			...
			if (dist <= engageRadius) {
				...
				const behindness = clamp(
					-(rx * tnx + ry * tny) / (engageRadius * 1.1),
					-1,
					1,
				);
				const sideFactor = clamp(
					Math.abs(rx * pnx + ry * pny) / engageRadius,
					0,
					1,
				);
				const alignForPull = clamp(
					0.18 + Math.max(0, behindness) * 1.05,
					0.1,
					1.25,
				);
		...
		if (n > 0 && toTargetLen > 0.0001) {
			...
			this.logoCarry.logoVx =
				this.logoCarry.logoVx * damping + (tnx * drive + pnx * latDrive) / mass;
			this.logoCarry.logoVy =
				this.logoCarry.logoVy * damping + (tny * drive + pny * latDrive) / mass;
			const angularDamping = 0.84;
			const angularDrive = torque * 0.075 + wobble * 0.15;
			this.logoCarry.logoAngularV =
				this.logoCarry.logoAngularV * angularDamping + angularDrive;
		...
		this.logoCarry.displayAngularV +=
			(visAngleTarget - this.logoCarry.displayAngle) * angleSpring * dt;
		this.logoCarry.displayAngularV *= angleDrag;
		this.logoCarry.displayAngle += this.logoCarry.displayAngularV * dt;
		this.logoCarry.displayAngle = clamp(
			this.logoCarry.displayAngle,
			-0.95,
			0.95,
		);

		if (
			this.logoCarry.logoX > worldWidth + 28 ||
			this.logoCarry.logoY > worldHeight + 28
		) {
			this.logoCarry.hidden = true;
		}
	}

	private updateLogoCarryCrew(
		dt: number,
		worldWidth: number,
		worldHeight: number,
	): void {
		...
		if (this.logoCarry.detourAngle !== 0) {
			...
			if (Math.abs(this.logoCarry.detourAngle) < 0.02)
				this.logoCarry.detourAngle = 0;
		}

		if (this.logoCarry.pauseTimer <= 0) {
			// Ant-like re-grip: tiny pauses while carrying.
			this.logoCarry.pauseTimer =
				Math.random() < 0.35 ? randomRange(0.08, 0.2) : randomRange(0.55, 1.4);
		}
		...
		if (currentCarriers.length > need) {
			...
			const candidates = this.bugs
				...
				.sort((a, b) => {
					const da = Math.hypot(
						a.x - this.logoCarry.logoX,
						a.y - this.logoCarry.logoY,
					);
					const db = Math.hypot(
						b.x - this.logoCarry.logoX,
						b.y - this.logoCarry.logoY,
					);
					return da - db;
				})
				.slice(0, missing);
		...
		if (this.logoCarry.swapTimer <= 0) {
			...
			if (stillCarriers.length > 0) {
				const lagger =
					stillCarriers[Math.floor(Math.random() * stillCarriers.length)];
	...
	private computeCarryHeading(bug: BugEntity): number {
		...
		const baseSlot =
			((bug.id * 2654435761 + this.logoCarry.slotSeed) >>> 0) % n;
		const slotPhase = n <= 1 ? 0 : (baseSlot / n) * Math.PI * 2;
		const jitterRng =
			(((bug.id * 1664525 + this.logoCarry.slotSeed) >>> 0) % 1000) / 1000 -
			0.5;
		...
		const ringR =
			7.4 +
			(((bug.id * 1103515245 + this.logoCarry.slotSeed) >>> 0) % 4) * 0.75;
	...
	private computeCarryFaceHeading(bug: BugEntity): number {
		...
		const jitter =
			(((bug.id * 214013 + this.logoCarry.slotSeed) >>> 0) % 1000) / 1000 - 0.5;
	...
	private createBug(worldWidth: number, worldHeight: number): BugEntity {
		...
		const speedBias =
			behaviorType === "loner" ? 1.15 : behaviorType === "social" ? 0.95 : 0.9;
		const speed = randomRange(BUG_MIN_SPEED, BUG_MAX_SPEED) * speedBias;
		const heading = randomRange(0, Math.PI * 2);
		const x = randomRange(
			BUG_BORDER_PADDING,
			Math.max(BUG_BORDER_PADDING + 1, worldWidth - BUG_BORDER_PADDING),
		);
		const y = randomRange(
			BUG_BORDER_PADDING,
			Math.max(BUG_BORDER_PADDING + 1, worldHeight - BUG_BORDER_PADDING),
		);
		...
		const color =
			BUG_MORANDI_COLORS[Math.floor(Math.random() * BUG_MORANDI_COLORS.length)];
		return {
			id: this.nextId++,
			x,
			y,
