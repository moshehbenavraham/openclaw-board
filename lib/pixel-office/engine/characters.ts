...
function seatGridCol(seat: Seat): number {
	return Math.round(seat.seatCol);
}
function seatGridRow(seat: Seat): number {
	return Math.round(seat.seatRow);
}
	...
	switch (ch.state) {
			...
			if (ch.wanderTimer <= 0) {
				...
				if (walkableTiles.length > 0) {
					...
					if (
						...
						Math.random() < INTERACTION_CHANCE
					) {
						const ip =
							interactionPoints[
								Math.floor(Math.random() * interactionPoints.length)
							];
						const path = findPath(
							ch.tileCol,
							ch.tileRow,
						...
						if (path.length > 0) {
							...
							ch.interactionTarget = {
								col: ip.col,
								row: ip.row,
								facingDir: ip.facingDir,
								furnitureType: ip.furnitureType,
							};
							ch.wanderCount++;
							ch.wanderTimer = randomRange(
								WANDER_PAUSE_MIN_SEC,
		...
		case CharacterState.WALK: {
			...
			if (ch.path.length === 0) {
				...
				if (ch.isActive) {
					if (!ch.seatId) {
						...
						if (
							seat &&
							ch.tileCol === seatGridCol(seat) &&
							ch.tileRow === seatGridRow(seat)
						) {
							ch.state = CharacterState.TYPE;
							ch.dir = seat.facingDir;
			...
			const speedMultiplier =
				ch.moveSpeedMultiplier > 0 ? ch.moveSpeedMultiplier : 1;
			ch.moveProgress +=
				((WALK_SPEED_PX_PER_SEC * speedMultiplier) / TILE_SIZE) * dt;
			...
			if (ch.isActive && ch.seatId) {
				...
				if (seat) {
					...
					if (
						!lastStep ||
						lastStep.col !== seatGridCol(seat) ||
						lastStep.row !== seatGridRow(seat)
					) {
...
function updateCat(
	...
	const catSpeed =
		WALK_SPEED_PX_PER_SEC * CAT_WALK_SPEED_FACTOR * rageSpeedFactor;
