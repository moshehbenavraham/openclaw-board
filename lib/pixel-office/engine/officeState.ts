...
import {
	CHARACTER_PALETTES,
	getAvailableCharacterVariantCount,
...
const PHOTO_COMMENTS_EN = [
	...
	"The decisive moment!!!",
	"Complex comp, nailed it!",
];

function getCodeSnippets(_locale: OfficeLocale): string[] {
	return CODE_SNIPPETS_EN;
}
function getSreBlackwords(_locale: OfficeLocale): string[] {
	return SRE_BLACKWORDS_EN;
}
function getPhotoComments(_locale: OfficeLocale): string[] {
	return PHOTO_COMMENTS_EN;
}
function getTempWorkerLabel(_locale: OfficeLocale): string {
	return "Temp";
}
function getGatewaySreLabel(_locale: OfficeLocale): string {
	return "On-Call SRE";
}
...
const _GATEWAY_SRE_LABEL = "On-Call SRE";
...
export class OfficeState {
	...
	private getSubagentSpawnCandidates(): Array<{ col: number; row: number }> {
		...
		return source.slice().sort((a, b) => {
			const da =
				Math.abs(a.col - SUBAGENT_SPAWN_CENTER_COL) +
				Math.abs(a.row - SUBAGENT_SPAWN_CENTER_ROW);
			const db =
				Math.abs(b.col - SUBAGENT_SPAWN_CENTER_COL) +
				Math.abs(b.row - SUBAGENT_SPAWN_CENTER_ROW);
			return da - db;
		});
	}
	...
	private pickDiversePalette(): { palette: number; hueShift: number } {
		...
		const extraVariants = available.filter(
			(index) => index >= CHARACTER_PALETTES.length,
		);
		const preferred =
			minCount === 0 && extraVariants.length > 0 ? extraVariants : available;
	...
	addAgent(
		...
		if (spawnAtDoor && this.doorwayTiles.length > 0 && seatId) {
			const door =
				this.doorwayTiles[Math.floor(Math.random() * this.doorwayTiles.length)];
			const seat = this.seats.get(seatId)!;
			seat.assigned = true;
			...
			const path = findPath(
				door.col,
				door.row,
				Math.round(seat.seatCol),
				Math.round(seat.seatRow),
				this.tileMap,
				this.blockedTiles,
			);
			if (path.length > 0) {
	...
	toggleFirstLobsterRage(): boolean {
		...
		if (!lobster?.isLobster) return false;
	...
	ensureGatewaySre(): void {
		...
		if (this.characters.has(id)) return;
		const spawn = this.findClosestWalkable(
			GATEWAY_SRE_STANDBY_COL,
			GATEWAY_SRE_STANDBY_ROW,
		);
	...
	private findClosestWalkable(
		...
		let bestDist =
			Math.abs(best.col - targetCol) + Math.abs(best.row - targetRow);
	...
	private getGatewaySrePatrolTiles(): Array<{ col: number; row: number }> {
		...
		const officeTiles = this.walkableTiles.filter(
			(t) => t.row <= 8 && t.col >= 1 && t.col <= 19,
		);
	...
	private getGatewaySreDegradedTiles(
		...
		const nearby = patrolTiles.filter(
			(t) =>
				Math.abs(t.col - rescuePoint.col) + Math.abs(t.row - rescuePoint.row) <=
				4,
		);
	...
	private updateGatewaySreCharacter(ch: Character, dt: number): void {
		...
		const degradedTiles = this.getGatewaySreDegradedTiles(
			patrolTiles,
			rescuePoint,
		);
		...
		if (this.gatewaySreStatus === "down") {
			...
			const atRescue =
				ch.tileCol === rescuePoint.col && ch.tileRow === rescuePoint.row;
			const last = ch.path[ch.path.length - 1];
			if (
				!atRescue &&
				(!last || last.col !== rescuePoint.col || last.row !== rescuePoint.row)
			) {
				this.withOwnSeatUnblocked(ch, () =>
					this.forceWalkTo(ch, rescuePoint.col, rescuePoint.row),
				);
			}
			this.withOwnSeatUnblocked(ch, () =>
				updateCharacter(
		...
		const scope =
			this.gatewaySreStatus === "degraded" ? degradedTiles : patrolTiles;
		this.withOwnSeatUnblocked(ch, () =>
			updateCharacter(
				ch,
				dt,
	 ...
	 */
	private updateHunterLobster(
		ch: Character,
		dt: number,
		idleTarget: Character | null,
	): void {
		if (!idleTarget) {
			this.withOwnSeatUnblocked(ch, () =>
				updateCharacter(
		...
		if (Math.abs(distPx - targetDistancePx) <= TILE_SIZE * 0.2) {
			...
			const last = ch.path[ch.path.length - 1];
			const keepsCurrentPath =
				last && last.col === destination.col && last.row === destination.row;
			if (!keepsCurrentPath) {
	...
	getSeatAtTile(col: number, row: number): string | null {
		for (const [uid, seat] of this.seats) {
			if (Math.round(seat.seatCol) === col && Math.round(seat.seatRow) === row)
				return uid;
		}
		return null;
	}
	...
	addSubagent(parentAgentId: number, parentToolId: string): number {
		...
		const availablePrioritySeats = SUBAGENT_PRIORITY_SEAT_IDS.filter(
			(seatId) => {
				const seat = this.seats.get(seatId);
		...
		if (availablePrioritySeats.length > 0) {
			const randomIdx = Math.floor(
				Math.random() * availablePrioritySeats.length,
			);
	...
	update(dt: number): void {
		...
		for (const ch of this.characters.values()) {
			...
			if (ch.isLobster) {
				...
				ch.lobsterBubbles = ch.lobsterBubbles.filter(
					(b) => b.age < LOBSTER_BUBBLE_LIFETIME_SEC,
				);
			}
			...
			if (
				...
				ch.photoComments = ch.photoComments.filter(
					(pc) => pc.age < PHOTO_COMMENT_LIFETIME,
				);
				if (
					ch.photoComments.length < 2 &&
					Math.random() < dt * PHOTO_COMMENT_SPAWN_RATE
				) {
					ch.photoComments.push({
						text: getPhotoComments(this.locale)[
							Math.floor(Math.random() * getPhotoComments(this.locale).length)
						],
						age: 0,
			...
			// - Gateway SRE in down state: ops slang
			...
			if (!ch.isCat && !ch.isLobster) {
				...
				ch.codeSnippets = ch.codeSnippets.filter(
					(s) => s.age < CODE_SNIPPET_LIFETIME,
				);
			}

			if (isSreFirefighting && !ch.isCat && !ch.isLobster) {
				if (
					ch.codeSnippets.length < 3 &&
					Math.random() < dt * SRE_BLACKWORD_SPAWN_RATE
				) {
					ch.codeSnippets.push({
						text: getSreBlackwords(this.locale)[
							Math.floor(Math.random() * getSreBlackwords(this.locale).length)
						],
						age: 0,
				...
				if (
					ch.codeSnippets.length < 2 &&
					Math.random() < dt * CODE_SNIPPET_SPAWN_RATE
				) {
					ch.codeSnippets.push({
						text: getCodeSnippets(this.locale)[
							Math.floor(Math.random() * getCodeSnippets(this.locale).length)
						],
						age: 0,
						x: (Math.random() - 0.5) * 20,
						y: 0,
