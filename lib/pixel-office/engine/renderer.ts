...
export function renderTileGrid(
	...
	const _GRID_STEP = 3; // Draw grid lines every N tiles
...
export function renderScene(
	...
	for (const ch of characters) {
		...
		if (ch.isSubagent && ch.state === CharacterState.TYPE && ch.seatId) {
			...
			const laptopWorldY =
				ch.y + sittingOffset + dy * forwardOffsetPx - 5 - TILE_SIZE / 8;
			...
			const laptopRotation =
				laptopFacing === Direction.DOWN
					? 0
					: laptopFacing === Direction.LEFT
						? 90
						: laptopFacing === Direction.UP
							? 180
							: 270;
		...
		if (ch.label) {
			...
			let labelColor = ch.isSubagent
				? isWorking
					? `rgba(220,38,38,${labelAlpha})`
					: "#991B1B"
				: isWorking
					? `rgba(34,197,94,${labelAlpha})`
					: "#FFD700";
...
export function renderPhotoComments(
	...
	const lifetime = 4.0;
	const _canvasH = ctx.canvas.height / (window.devicePixelRatio || 1);
	for (const ch of characters) {
