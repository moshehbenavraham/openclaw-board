...
function shouldRemoveRightOfficeLegacyItems(item: PlacedFurniture): boolean {
	...
	if (
		...
		return true;
	if (item.type === FurnitureType.PLANT && item.col === 19 && item.row === 3)
		return true;
	if (item.type === FurnitureType.LAMP && item.col === 19 && item.row === 7)
		return true;
	if (item.type === FurnitureType.COOLER && item.col === 18 && item.row === 7)
		return true;
	return false;
}

function normalizeRightOfficeFurniture(
	furniture: PlacedFurniture[],
): PlacedFurniture[] {
	const base = furniture.filter(
		(item) => !shouldRemoveRightOfficeLegacyItems(item),
	);
	const next = [...base];
	...
	for (let r = 0; r < rows; r++) {
		for (let c = 0; c < cols; c++) {
			...
			if (c >= 8 && c <= 12 && r >= 13 && r <= 15) {
				...
				tiles.push(F1);
				tileColors.push(DEFAULT_LOUNGE_COLOR);
			}
		}
	}
...
const INTERACTABLE_TYPES = new Set([
	...
	FurnitureType.WATER_COOLER,
	FurnitureType.BOOKSHELF,
	FurnitureType.LIBRARY_GRAY_FULL,
...
export function getDoorwayTiles(
	layout: OfficeLayout,
